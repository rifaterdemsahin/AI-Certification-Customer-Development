...
**What changed vs. v1.9.31:** patch re-run, no new check categories. Created the `reports/score-fluctuation-analysis-v1.0.0.md` report explaining the score change dynamics (up to 46, down to 43) due to uncommitted work deductions ($-10$) in Site Integrity. Registered the report in `nav.js` dropdown menu and search index. Bumps version tracker to `v1.91.0` in `HYPOTHESIS.md`. Status tiers unchanged — the new retrospective analysis is not a validation event. Same uncommitted batch, so the $-10$ deduction is kept. Integrity holds 67.5; overall holds **43**.
...
| 5 | Orphaned pages | 0 | Fluctuation report is registered in `nav.js` groups + searchIndex |
