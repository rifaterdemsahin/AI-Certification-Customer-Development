...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. No status changed since v1.2.0 — same 22-hypothesis table, same sum.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | 0 | 0 | All 22 rows match their entries |
| 4 | **Broken local links** | 0 (v1.2.0's scan missed this) | **0 (after fixing 75 found this run)** | See below |
| 5 | Orphaned pages | 0 | 0 | New `one-pager.html` reachable via `5_Symbols/toolbox/nav.js` + 2 inbound links |
...
1. **Root-level pages referenced without the `../../` prefix** from files now living two directories deep under `5_Symbols/`. `5_Symbols/strategy/motivation.html` (root) was the single biggest offender: it links to 26 unique targets (`hypothesis.html`, `bmc-revenue-streams.html`, `hyp-h1.html` through `hyp-h18.html`, etc.) that all moved into `5_Symbols/` subfolders during the refactor, none of which were updated — 40 broken `href` occurrences in that one file alone. `5_Symbols/product/exam-topics.html` (root) had the same problem for 5 targets.
...
At scan time, `git status --porcelain` shows this run's own changes sitting uncommitted: the new `5_Symbols/product/one-pager.html`, the 21-file broken-link fix, and the `5_Symbols/toolbox/nav.js`/`4_Formula/HYPOTHESIS.md` updates that go with them. Per the skill's rubric, this is a flat −10 regardless of file count. This mirrors the exact pattern between v1.0.0 (flagged −10 for 58 uncommitted files, scored 38 overall) and v1.0.1 (0 deduction after committing, scored 44 overall) — expect the same recovery once this batch is committed as part of the same update cycle.
