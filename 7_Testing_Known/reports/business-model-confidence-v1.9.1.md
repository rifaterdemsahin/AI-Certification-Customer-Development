...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. All 25 hypotheses hold the same status emoji as v1.9.0 — H5, H8, and H14 gained new curriculum-scope evidence text (the 7-cohort Skool sequence) but none crossed a status threshold, since enrollment/conversion/multi-vendor-content data is still unmeasured.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | 0 | 0 | 0 | All 25 rows match their entries. |
...
| 5 | Orphaned pages | 0 | 0 | 0 | Full reachability scan confirms every page (including the new `sales-marketing-roadmap.html`) is reachable via `5_Symbols/toolbox/nav.js` or an `href`. |
| 6 | Headline number cross-file consistency | 0 | 0 | 0 | Consistent — no stray currency/number variants found live on any page. |
| 7 | Uncommitted work (`git status --porcelain`) | −10 | −10 | −10 | New, distinct batch: `skool-lms-integration.html`, `skool-vs-youtube.html`, `4_Formula/HYPOTHESIS.md`, `5_Symbols/toolbox/nav.js` modified and `sales-marketing-roadmap.html` added, all part of the same 7-cohort Skool curriculum feature. Will clear once committed. |
...
| v1.5.7 and earlier | — | — | See `7_Testing_Known/reports/business-model-confidence-v1.9.0.md` for the full prior history. |
