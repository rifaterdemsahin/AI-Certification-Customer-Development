...
- One branch added `5_Symbols/dashboard/sitemap.html` (a searchable Site Map, no new hypothesis) and, prompted by that change, ran a fresh full-repo re-scan that caught 2 pre-existing findings missed by earlier runs — 3 real broken links and H24 missing from `4_Formula/HYPOTHESIS.md`'s own Summary Table — but left them **flagged, not fixed**, to keep that batch scoped. That run scored **38/100**.
...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md` (not the Summary Table). Unchanged from the H25 branch's own v1.7.0 table — the Site Map branch added no hypothesis, so merging it doesn't move this score.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | −5 (H24 missing) | 0 (fixed) | 0 | 0 | The Site Map branch found but didn't fix H24's missing Summary Table row; the H25 branch fixed it. Only the H25 branch's commits touched `4_Formula/HYPOTHESIS.md`'s table, so the fix carries through the merge untouched. All 25 rows now match their entries. |
...
| 5 | Orphaned pages | 0 | 0 | 0 | 0 | All 3 pages added across both branches (`sitemap.html`, `cert-value-ai-era.html`, `hyp-h25.html`) are registered in `5_Symbols/toolbox/nav.js` and reachable via at least one inbound link each. Full reachability scan: 0 orphans. |
