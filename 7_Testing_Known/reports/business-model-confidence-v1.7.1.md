...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. No status tier changed since v1.7.0 — table unchanged. This batch added the new page's path to H4, H5, H7, and H21's `Source:` lines only; none of their `Status:` lines moved.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | −5 | −5 | −5 | Still open — `H24` has a full entry but no Summary Table row (table still lists H1–H23 only, 23 of 24 tracked hypotheses). Not touched by this batch. |
...
| 5 | Orphaned pages | 0 | 0 | 0 | 0 orphans found. The new page is registered in `5_Symbols/toolbox/nav.js`'s Growth dropdown and search index, added to `latest-pages.html`'s curated list, and cross-linked from `marketing-tactics.html` — reachable by both `href` and `5_Symbols/toolbox/nav.js`. |
...
| 7 | Uncommitted work (`git status --porcelain`) | 0 | 0 | 0 | This batch (`youtube-titles-skool-mapping.html` new; `5_Symbols/toolbox/nav.js`, `marketing-tactics.html`, `latest-pages.html`, `4_Formula/HYPOTHESIS.md` edited for registration/cross-linking) is committed as part of this same run |
