...
Bumped `4_Formula/HYPOTHESIS.md` to **v1.163.0**. Overall score recovers **33 → 49 / 100**. This is the integrity-ceiling score given today's Validation (37.1) and remaining acidity deductions (22.5). It is *not* a business-validation jump — no hypothesis status tier changed.
...
| 5 | Orphaned pages | 0 | Fresh scan of 219 HTML files: every page is reachable via `5_Symbols/toolbox/nav.js` groups/searchIndex or an inbound link. `how-to-move-the-score.html` and `hyp-h27.html` are registered. |
