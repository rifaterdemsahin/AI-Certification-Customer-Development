...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | 0 | All 27 Summary Table rows match their Status-line leading emoji |
| 4 | Broken local links | 0 | Fresh scan, 159 HTML files, `<script>` stripped: 0 real broken `href`s. `signal-versus-noise.html` scored URLs all resolve (H27 detail page does not exist; that row points at `4_Formula/HYPOTHESIS.md`). |
| 5 | Orphaned pages | 0 | New page is in `5_Symbols/toolbox/nav.js` Strategy dropdown + searchIndex + hub + inbound links from focus, evidence-map, sitemap, latest-pages, dictionary, and H9. |
