> **Superseded by** [`7_Testing_Known/reports/business-model-confidence-v1.9.20.md`](business-model-confidence-v1.9.20.md). Numbers below are the historical record for this run and are not edited in place.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | 0 | All 28 Summary Table rows match their own Status-line leading emoji, including H5 🟡 on both sides |
...
| 7 | Uncommitted work (`git status --porcelain`) | −10 | Same open batch as v1.9.16 / v1.9.17 / v1.9.18 (after commit `9d748bc`). This run adds the H5 named-pair edits (hyp-h5, 4_Formula/HYPOTHESIS.md, H20, organic-growth, evidence-map, etc.) to that tree. Does not stack a second −10. Clears once the tree is committed (Integrity → 77.5, overall → 46). |
