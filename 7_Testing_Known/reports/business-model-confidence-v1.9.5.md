...
**What changed vs. v1.9.4:** Added `5_Symbols/product/youtube-skool-handoff.html` documenting the fixed platform order: YouTube/LinkedIn = acquisition; Skool = Stage 2 delivery (Classroom + forum + calendar + billing); CTAs/warm invites = the handoff bridge. Registered in `5_Symbols/toolbox/nav.js`, linked from LMS / Skool-vs-YouTube / organic-growth / posting-sanity-check / latest-pages. Source lines updated on H4, H5, H7, H8. No new hypothesis; no hypothesis status-tier change.
...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. All 27 hypotheses keep the same status emoji as v1.9.4.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | 0 | No new mismatches introduced. |
| 4 | Broken local links | 0 | New handoff page: 0 broken local hrefs. Prior H27 → `practice-exams-gap.html` fix still holds. |
| 5 | Orphaned pages | 0 | `youtube-skool-handoff.html` in `5_Symbols/toolbox/nav.js` groups + searchIndex; inbound links from LMS, skool-vs-youtube, organic-growth, posting-sanity-check, latest-pages. |
