> **Superseded by** [`7_Testing_Known/reports/business-model-confidence-v1.9.8.md`](business-model-confidence-v1.9.8.md). Numbers below are the historical record for this run — do not edit in place.

# Business Model Confidence Report — v1.9.7

**Date:** 2026-08-13  
**Produced by:** the `business-model-sanity-check` skill (`.claude/skills/business-model-sanity-check/SKILL.md`)  
**What changed vs. v1.9.6:** patch bump — named **R.A.I.S.E.** (Rapid Artificial Intelligence Increases Skills Expectations) as the starting-point framing for H1 and the founder origin story. Created `5_Symbols/strategy/raise.html` and cross-linked it from motivation, certification, skills-gap, pain-points, idea, H1, the dictionary, nav, and the hub. No hypothesis status-tier change.
...
> Hypothesis Validation holds at **30.0/100** (27 hypotheses; no status emoji changes). Site Integrity holds at **67.5/100** (0 broken links; 0 orphans; −10 uncommitted-work for this distinct R.A.I.S.E. + prior Sude-archive batch; acidity findings carried forward). Overall remains **41**. A low-moderate score at this stage is expected — most Stage 2/3 bets are still open.
...
| Sub-score | v1.9.6 | v1.9.7 (this run) |
|---|---|---|
...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. All 27 hypotheses keep the same status emoji as v1.9.6.

| ID | Hypothesis (one line) | Status | Score |
|---|---|---|---|
| H1 | Rising AI skills expectations (now named R.A.I.S.E.) | 🟡 Partially validated | 55 |
...
H1 received a named framing (R.A.I.S.E.) and a founder-origin page this run. That is positioning, not new market evidence — status stays 🟡 Partially validated.
...
| 3 | 4_Formula/HYPOTHESIS.md table/entry mismatches | 0 | Leading status emoji matches the Summary Table for every hypothesis. |
| 4 | Broken local links | 0 | Fresh scan of all `*.html` hrefs (scripts stripped): 0 broken local targets. |
| 5 | Orphaned pages | 0 | Every `*.html` except `index.html` is reachable via an href or `5_Symbols/toolbox/nav.js`. `raise.html` is in the Strategy dropdown, search index, and linked from multiple pages. |
...
| 7 | Uncommitted work (`git status --porcelain`) | −10 | Distinct new batch: R.A.I.S.E. starting-point page plus the still-uncommitted Sude-archive follow-up. Clears once this combined working-tree batch is committed. |

**Total deductions: 32.5 → Site Integrity Score: 67.5**

Uncommitted-work grouping (this batch):

- New: `5_Symbols/strategy/raise.html`, `5_Symbols/strategy/raise-hero.jpg`, `7_Testing_Known/reports/business-model-confidence-v1.9.7.md`.
- Cross-links: `5_Symbols/strategy/motivation.html`, `why-certification.html`, `cert-value-ai-era.html`, `skills-gap.html`, `pain-points.html`, `idea.html`, `hyp-h1.html`, `hypothesis.html`, `evidence-map.html`, `dictionary.html`, `index.html`, `5_Symbols/toolbox/nav.js`, `latest-pages.html`, `CLAUDE.md`, `gemini.md`, `4_Formula/HYPOTHESIS.md`.
- Still sitting from the prior Sude-archive feature: interview archive pages, H5/H8/H20, behavior report v1.4.0, and `business-model-confidence-v1.9.6.md`.
...
**R.A.I.S.E. is now the named starting point, not a new demand claim.**

The page records founder Rifat Erdem Sahin's Information Technology background, his past pain with learning systems, the choice of proctored exams as complexity increases, and the authentic teaching position of pushing AI to its limits for self-learning. It cites existing H1 / H2 / H8 / H24 / H25 rather than asserting a 28th hypothesis.
...
|---|---|---|---|
| **Hypothesis Validation (30 → higher)** | Average of 27 statuses | 100 if all ✅ | Highest leverage: **H9** two consecutive paid cohort launches combining to **$10,000**. Next cheap move: **H27** question bank well past 2/60 with logged attendee demand. **H5/H8** need paid enrollment and non-founder Skool threads after warm handoff. **H20** needs a counted referral. **H1** would move only with an exported search-volume/job-scrape series — naming R.A.I.S.E. does not raise it. |
...
| **Uncommitted work (−10)** | R.A.I.S.E. + Sude-archive batch uncommitted | 0 | Commit raise.html + cross-links + prior archive/report files. Site Integrity → **77.5**, overall → **44**. |
| **Broken links / orphans** | Already 0 | Hold | Keep registering new pages in `5_Symbols/toolbox/nav.js` before considering them done. |
...
3. **H1** still needs an exported numeric search-volume series, not more framing.
