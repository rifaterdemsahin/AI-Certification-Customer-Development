> **Superseded by** [`7_Testing_Known/reports/business-model-confidence-v1.9.9.md`](business-model-confidence-v1.9.9.md). Numbers below are the historical record for this run — do not edit in place.
...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. All 27 hypotheses keep the same status emoji as v1.9.7.
...
| 5 | Orphaned pages | 0 | Every `*.html` except `index.html` is reachable via an href or `5_Symbols/toolbox/nav.js`. `vc-feedback.html` is in the Strategy dropdown, search index, hub investor card, and linked from the deck, pitch, moat, risk register, H22, and dictionary. |
...
- **This run:** `5_Symbols/product/vc-feedback.html`, `5_Symbols/product/vc-feedback-hero.jpg`, plus cross-links on `vc-deck.html`, `pitch-deck.html`, `risk-analysis.html`, `moat.html`, `hyp-h22.html`, `dictionary.html`, `index.html`, `5_Symbols/toolbox/nav.js`, `latest-pages.html`, `4_Formula/HYPOTHESIS.md`, `CLAUDE.md`, `gemini.md`.
...
| **Broken links / orphans** | Already 0 | Hold | Keep registering new pages in `5_Symbols/toolbox/nav.js` before considering them done. Do not link `hyp-h27.html` until that detail page exists. |
