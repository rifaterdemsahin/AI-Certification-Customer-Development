...
Re-derived fresh from each hypothesis's own **Status:** line in `4_Formula/HYPOTHESIS.md`. No status tier changed since v1.6.5 — table unchanged from that version.
...
| 5 | Orphaned pages | 0 | 0 | 0 | 0 orphans found — every one of the 144 HTML files (except `index.html`) is reachable via `href` or `5_Symbols/toolbox/nav.js`. `sitemap.html` itself confirmed reachable (registered in `5_Symbols/toolbox/nav.js`, linked from `index.html` and `latest-pages.html`). |
| 6 | Headline number cross-file consistency | 0 | 0 | 0 | $10,000 / 1,000x / $100,000 / >40% all consistent; no stray currency variants found |
| 7 | Uncommitted work (`git status --porcelain`) | −10 | 0 | 0 | Cleared — this run's batch (`sitemap.html` new, plus `5_Symbols/toolbox/nav.js`, `index.html`, `latest-pages.html` edited for registration/cross-linking) is committed as part of this same run |
