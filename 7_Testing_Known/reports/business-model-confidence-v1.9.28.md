...
**What changed vs. v1.9.27:** patch re-run, no new check categories. Founder confirmed the live Delivery Pilot checkout as **Freemium** with entitlements — Standard $0 (posts, learning materials, welcome) / Premium $1/mo (recorded workshops + listen-only Sunday) / VIP $250/year (screen-share, personal architecture assistance, founder setup reviews). Versioned as `7_Testing_Known/reports/skool-pricing-feedback-v1.0.0.md`. Names still Standard / Premium / VIP; recommended Peek / Sit In / Share Screen. VIP 1:1 capped as Sunday-in-the-room. Not a paid-enrollment signal. Status tiers unchanged. This run commits the Delivery Pilot / pricing-feedback batch. A separate untracked draft (`5_Symbols/product/skool-discoverability-keywords.html`) remains, so the −10 uncommitted-work category still applies.
...
Re-derived from each hypothesis's own **Status:** line. All 28 keep the same leading emoji as v1.9.27. Sum 900 / 28 = **32.14**. H5 Premise 9 now records founder-confirmed Freemium entitlements and cites `7_Testing_Known/reports/skool-pricing-feedback-v1.0.0.md`. Status tier unchanged — confirming prices is not a paid enrollment.
...
| 5 | Orphaned pages | 0 | Pricing feedback is a markdown report, registered in `5_Symbols/toolbox/nav.js` groups + searchIndex and inbound-linked from offer, H5, hub, dictionary, latest-pages, revenue streams |
